...
class JobGetScheduleResponse extends MainResponseClass
{
     ...
     *  - spentTime - сколько времени потратил за день в минутах (пользователь заполняет самостоятельно)
...
if (!empty(trim($in->filterEndDate))) {
    $filterEndDate = date_create($in->filterEndDate, new DateTimeZone("Europe/Moscow")); // при неправильной дате принимает значение false
}
...
$stmt = $pdo->prepare("
    SELECT `managers_job_days`.`id`, `managers_job_days`.`date`, `managers_job_days`.`report`, `managers_job_days`.`spent_time`, `managers_job_days`.`is_weekend`, `managers_job_days`.`comment`
...
while ($daysData = $stmt->fetch(PDO::FETCH_ASSOC)) {
    $days[] = [
        ...
        'spentTime' => (string)$daysData['spent_time'],
