<?php //Основные классы для API
class API_root_class{
	public $signature='';
	
	function calc_signature($nasotku_token){
		...
		$sign_calc=hash('sha256', $concat);
		return $sign_calc;
	}
	
	//проверяет подпись в запросе
	function check_signature($nasotku_token){
	...
	function make_wrong_resp($message, $suc = "0"){//Подпись здесь будет неверная
